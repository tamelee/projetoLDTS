...
public class MenuWinnerController extends Controller<MenuWinner> {

    public MenuWinnerController(MenuWinner menu){
        super(menu);
    }
    ...
    public void step(Application game, GUI.ACTION action, long time) throws IOException, UnsupportedAudioFileException, LineUnavailableException {
        switch(action) {
            ...
            case SELECT -> {
                ...
                if(getModel().isSelectedScores()) game.setState(new MenuScoreState(new MenuInfo(game.getArena(), new ScoreReader(game.getArena()))));
